...
from ambient_scribe.models import NoteRequest, NoteResponse, SuggestionResponse, ErrorResponse
...
async def debug_transcripts():
    ...
    return {
        ...
        "transcripts": {tid: {"filename": t.filename, "segments": len(t.segments)} for tid, t in _transcripts.items()}
    }
...
async def stream_note_endpoint(note_request: NoteRequest):
    ...
    print(f"POST /stream - Received request: transcript_id={note_request.transcript_id}, template={note_request.template_name}")
        ...
        try:
            ...
            async for event in generate_note_service(
                ...
                if event.get('type') == 'complete':
                    ...
                    print(f"Captured final note markdown, length: {len(final_note_markdown) if final_note_markdown else 0}")
