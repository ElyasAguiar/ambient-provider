    ...
    try:
        ...
        if file_path.exists():
            try:
                ...
                logger.error(
                    f"Failed to cleanup file: {file_path}, error: {cleanup_exc}",
                    exc_info=True,
                )
    ...
    try:
        ...
        async def stream_transcription():
            try:
                ...
                if (
                    last_result
                    and last_result.get("type") == "complete"
                    and "transcript" in last_result
                ):
                    ...
                    logger.info(
                        f"Streaming transcription completed: transcript_id={transcript_id}"
                    )
        ...
        logger.error(
            f"Exception during streaming transcription setup: {e}", exc_info=True
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Streaming transcription setup failed: {str(e)}"
        )
...
async def list_transcripts() -> list[Transcript]:
    ...
    for transcript_id, transcript in _transcripts.items():
        # Fix audio URL format if it doesn't have the /api prefix
        if transcript.audio_url and not transcript.audio_url.startswith(
            "/api/uploads/"
        ):
            if transcript.audio_url.startswith("/uploads/"):
                transcript.audio_url = "/api" + transcript.audio_url
